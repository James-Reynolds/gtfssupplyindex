...
GTFS data is first loaded, with the gtfs_by_route_type function splitting this into a list of tidygtfs objects, with one list element for each route_type present in the gtfs data. This is achieved using the filter_by_route_type function from the gtfstools package [@filter_GTFS_by_mode].
...
```{r fulter_buffer_distance, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE, out.width='100%', fig.margin = TRUE}
...
The SI_calc.R function takes the stops_in_or_near_areas list (by route_type) of dataframes and the list_gtfs list (by route_type) of tidygtfs objects and calculates the si_by_mode_and_time list (by route_type) of dataframes. This dataframe has a record for each area_id and combination of date, start_time and end_time specified by the user, and the corresponding (calculated) SI value.   

The tidytransit package includes an article about generating a departure timetable[@tidytransit_departure_timetable]. Code was adapted from this example to create functions to calculate the number of departures and arrivals from (a vector of) stop_ids within a given time period. 

```{r departures_mornington_20181230, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}

stop_ids <- gtfs_single_route_type$stops %>%
 dplyr::select(stop_id)

departures_by_stop_id <- gtfssupplyindex::departures(
 gtfs = gtfs_single_route_type,
 stop_ids = stop_ids,
 date_ymd = "2018-12-30",
...
departures_by_stop_id %>% kable(caption = "Departures at each stop for Sunday December 2018, Mornington Peninsula tourist railway.")
...
All the inputs to calculate the si_by_mode_and_time list (by route_text) of tables are now available. Hence, the SI.calc function can be run, as follows:

```{r SI_mornington_20181230, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
...
The si_by_mode_and_time has a nested list structure, with the levels showing (in order), the date, the start time, the end time, the route_type. The elements of the final list (route_type) contain a dataframe with the area_id and SI, in this instance for the 'rail' route_type relevant to the Mornington Peninsula Railway. 

Calculation of the SI for other time periods can be simply appended, as follows. 

```{r SI_mornington_20181230, echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
####-----first load all the inputs

si_by_mode_and_time_1 <- SI_calc(
  list_gtfs = list_gtfs,
  stops_in_or_near_areas = stops_in_or_near_areas, 
  date_ymd = lubridate::ymd("2018-12-23"), #7 days earlier
  start_hms = lubridate::hms("10:30:00"), # Same start time
  end_hms = lubridate::hms("11:30:00"), # Only for one hour
verbose = TRUE)

si_by_mode_and_time <- append(si_by_mode_and_time, si_by_mode_and_time_1)

si_by_mode_and_time_2 <- SI_calc(
  list_gtfs = list_gtfs,
  stops_in_or_near_areas = stops_in_or_near_areas, 
  date_ymd = lubridate::ymd("2018-12-23"), #7 days earlier
  start_hms = lubridate::hms("11:30:00"), # Same start time
  end_hms = lubridate::hms("12:30:00"), # Only for one hour
verbose = TRUE)

si_by_mode_and_time <- append(si_by_mode_and_time, si_by_mode_and_time_2)
...
- Mornington Peninsula Tourist Railway and SA2 zones within the Mornington Peninsula SA3 zone
- Mornington Peninsula Tourist Railway and just the Mornington Peninsula SA3 zone 
- Victorian GTFS and the SA1 zones within the Melbourne CBD - North SA2 zones
...
```{r mornington_SA2_test data, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.margin = FALSE, fig.cap = c("Catchment of each stop within each area of interest, Mornington Penninsula Railway and SA2 zones", "Catchment of each stop within each area of interest, by stop, Mornington Penninsula Railway and SA2 zones", "Catchment of each stop within each area of interest, by stop (x) and area of interest (y), Mornington Penninsula Railway and SA2 zones")}
#load the revised mornington GTFS data
list_gtfs = gtfssupplyindex:::gtfs_by_route_type(system.file(
  "extdata/mornington180109",
  "gtfs.zip", 
...
Testing the code with the code with the same Mornington Peninsula Tourist Railway GTFS, but instead combining the areas_of_interest so that we are only looking at the Mornington Peninsular SA3 zone (in its entirety) gives^[Verbose set to false in this instance]:



```{r mornington_SA3_test data, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
### Victorian GTFS and SA1 zones within the Melbourne CBD - North SA2 zone
...
```{r Melbourne_CBD_north_test_check_whole_function data, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.margin = FALSE, fig.cap = c("Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams, by stop, first six stops", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, rail", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, rail, by stop, first six stops", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, bus", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, bus, by stop, first six stops")}
#load the Victorian GTFS
ptv_231228 <- gtfssupplyindex:::gtfs_by_route_type(path = "data/ptv_231228/gtfs.zip")
...
```{r Russell_bourke_map, fig.width = 10, fig.height = 2, fig.margin = TRUE, fig.cap = "Area mapping for stop 17591 (Russel and Bourke), duplicated from above verbose code output", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, out.width='100%'}
knitr::include_graphics("graphics/russell_bourke_map.png")
