...
stops_in_or_near_areas <- gtfssupplyindex:::stops_in_walk_dist(
  list_gtfs = ptv_231228_trams_only, 
  areas_of_interest = areas_of_interest,
  EPSG_for_transform = 28355,
  verbose = TRUE
)
...
```{r Melbourne_CBD_north_test_check_whole_function_rail, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.margin = FALSE, fig.cap = c("Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams, by stop, first six stops"}
ptv_231228_rail_only <- ptv_231228[2]
...
stops_in_or_near_areas <- gtfssupplyindex:::stops_in_walk_dist(
  list_gtfs = ptv_231228_rail_only, 
  areas_of_interest = areas_of_interest,
  EPSG_for_transform = 28355,
  verbose = TRUE
)
```
The rail results appear to meet expectations: 
...
```{r Melbourne_CBD_north_test_check_whole_function_all, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.margin = FALSE, fig.cap = c("Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams", "Catchment of each stop within each area of interest, Melbourne CBD - North SA1 zones, trams, by stop, first six stops"}
...
si_by_area_and_hour_wider <- hourly(list_gtfs = list_gtfs,
                                    stops_in_or_near_areas = stops_in_or_near_areas, 
                                    date_ymd = "2024-01-15")

si_by_area_and_hour_wider %>% adorn_totals(where = "col") %>% kable(caption = "SA1 zones within the Melbourne CBD - North SA2 zone, hourly SI values for January 15, 2024")
