...
(2) $Area_{Bn}$ is the buffer area for each stop (n) within the area of interest. In  
Currie and Senbergs (2007) this was based on 
...
The upper part of the ERD shows the GTFS structure, which remains unchanged.  Below this are the additional tables developed for the purposes of calculating the SI for each area of interest. The desired end result (bottom right) is the si_by_area_day_and_hour, which will report the SI score for each hour and date.  

As discussed above, the SI is addative, so the si_by_area_day_and_hour table can be calculated from a si_by_mode_and_time table containing individual SI scores by mode^[Because individual modes may have different buffer distances.] for individual periods of time. 
