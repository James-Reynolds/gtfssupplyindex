...
si_by_area_and_hour_wider %>% select(1, 6:28) %>% head(n=75) %>% adorn_rounding(digits = 0) %>% kable(caption = "Victorian GTFS and SA1 zones within the Greater Melbourne GCCSA, hourly SI values for October 10, 2023, first 75 SA1 zones, 4am to 2am only" )
...
The below figure maps this output for just the SA1 zones within the Melbourne CBD - North SA2 zone. 
...
#Join SI to map data
map_si_by_area_and_hour <- inner_join(map_areas_of_interest, si_by_area_and_hour)

map_si_by_area_and_hour$hour_starting <- as_factor(map_si_by_area_and_hour$hour_starting)
...
```{r Caulfield_campus_all_map_231006, echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.fullwidth = TRUE, fig.cap="Victorian rail and SA1 zones within the Melbourne CBD - North SA2 zone,SI values for January 16, 2024, by hour"}
