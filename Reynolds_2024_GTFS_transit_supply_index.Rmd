...
```{r SI_ERD, fig.width = 10, fig.height = 2, fig.fullwidth = TRUE, fig.cap = "SI data structure entity relationship diagram. Source: adapted by author from Alamri et al (2023) and the GTFS Schedule Reference (16/11/2023 revision).", echo = FALSE, warning=FALSE, message=FALSE, cache=TRUE, out.width='100%'}
knitr::include_graphics("graphics/SI_data_structure.png")

```

The desired end result is the si_by_area_day_and_hour table (bottom right), which reports the SI score for each hour of the day across dates specified by the user. As discussed above, the SI is additive, so the si_by_area_day_and_hour table can be calculated from a si_by_mode_and_time table containing individual SI scores by mode^[SI calculations must be on a per mode basis up until the final aggregation because individual modes may have different buffer distances.] for individual periods of time. 
...
- load_buffer_zone.R, which obtains data about route_type, (mode) name and the assumed catchment areas (400 or 800m);
- load_areas_of_interest.R, which get geographic data about the areas of interest for which the SI calculation is to be performed; 
- the gtfs_by_mode.R function, which transforms the gtfs data into a list of tidygtfs objects, with an entry for each route_type (mode);
- the stops_in_walk_dist.R function, which determines which of the stops within the GTFS dataset is inside or within walking distance of the areas_of_interest boundaries - CURRRENTLY IN DEVELOPMENT, NEXT STEP ISTO FIGURE OUT HOW TO MAKE SURE THAT THE PROJECTION USED IS IN METRES, OR TRANSFORM IF NECESSARY. 
- the area_terms.R function, which calculates the $Area_{Bn}/Area_{area}$ component of the SI for each combination of stop, area of interest and route type (mode) - TO BE DEVELOPED; 
- arrivals.R, which calculates the number of service arrivals at a stop within a given time period on a specific date, being $SL_{n,time}$ component of the SI^[departures.R and arrivals.R are a generalisations of code in the tidytransit vignettes.] - TO BE DEVELOPED, JUST NEED TO ADAPT THE DEPARTURES.R; 
- SI_calc.R, which combines the various terms in the SI caluclation - TO BE DEVELOPED; and
- hourly.R, which provides a wrapper function to calculate hourly SIs for all areas of interests on a specific date or set of dates - TO BE DEVELOPED. 
