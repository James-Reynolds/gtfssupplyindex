...
departures <- function(gtfs = gtfs,
  ...
  departures <- departures %>% dplyr::filter(stop_sequence != last_stop_sequence_value)
  
  #  departures %>% dplyr::filter(trip_id == 47) %>% dplyr::select(stop_id, stop_sequence, departure_time, trip_destination, trip_destination_id, last_stop_sequence_value)
...
arrivals <- function(gtfs = gtfs,
  ...
  # remove rows where the stop sequence is equal to the last stop, as these services terminate
  arrivals <- arrivals %>% dplyr::filter(stop_sequence != first_stop_sequence_value)
  
  #  arrivals %>% dplyr::filter(trip_id == 47) %>% dplyr::select(stop_id, stop_sequence, departure_time, trip_destination, trip_destination_id, last_stop_sequence_value)
...
first_and_last_stops <- function(gtfs = gtfs) {
  ...
  first_stop_sequence_value <- gtfs$stop_times %>% 
    dplyr::group_by(trip_id) %>% 
    dplyr::summarise(min(stop_sequence))
  
  first_stop_id <- dplyr::left_join(first_stop_id, first_stop_sequence_value, by = "trip_id")
  names(first_stop_id) <- c("trip_id", "stop_id", "first_stop_sequence_value")
  
  # join with the stops table to get the stop_name
  first_stop_names <- dplyr::left_join(first_stop_id, gtfs$stops, by="stop_id")
  
  # rename the first stop_name as trip_origin and the first_stop as trip_origin_id
  trip_origins <- first_stop_names %>% dplyr::select(trip_id, trip_origin = stop_name, trip_origin_id = stop_id, first_stop_sequence_value)
  
  # join the trip origins back onto the trips
  gtfs$trips <- dplyr::left_join(gtfs$trips, trip_origins, by = "trip_id")
  
  gtfs$trips %>%
    dplyr::select(route_id, trip_origin, trip_origin_id, first_stop_sequence_value) %>%
    head()
  ...
  if(!exists("trip_headsign", where = gtfs$trips)) {
    ...
    gtfs$trips <- dplyr::left_join(gtfs$trips, trip_headsigns, by = "trip_id")
  }
  ...
  last_stop_sequence_value <- gtfs$stop_times %>% 
    dplyr::group_by(trip_id) %>% 
    dplyr::summarise(max(stop_sequence))
  
  last_stop_id <- dplyr::left_join(last_stop_id, last_stop_sequence_value, by = "trip_id")
  names(last_stop_id) <- c("trip_id", "stop_id", "last_stop_sequence_value")
  
  # join with the stops table to get the stop_name
  last_stop_names <- dplyr::left_join(last_stop_id, gtfs$stops, by="stop_id")
  
  # rename the last stop_name as trip_origin and the last_stop as trip_origin_id
  trip_destinations <- last_stop_names %>% dplyr::select(trip_id, trip_destination = stop_name, trip_destination_id = stop_id, last_stop_sequence_value)
  
  # join the trip origins back onto the trips
  gtfs$trips <- dplyr::left_join(gtfs$trips, trip_destinations, by = "trip_id")
  
  gtfs$trips %>%
    dplyr::select(route_id, trip_origin_id, trip_destination_id, trip_destination, last_stop_sequence_value) %>%
    head()
